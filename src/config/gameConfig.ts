import { validateConfig, deepMerge, validationRules } from './configValidator';
import { applyDataAttributes } from '../utils/dataAttributeParser';
import { GameConfig, GameData } from './types';

// Export GameConfig type explicitly
 ...
 * @param def - Default object to compare against
 * @param path - Current path in object hierarchy
 * @returns Object containing only the differences
 */
function compareObjects<T extends Record<string, any>>(current: T, def: T, path: string = ''): Partial<T> {
...
class ConfigManager {
    private sources: { default: GameConfig | null; localStorage: GameConfig | null; dataAttributes: GameConfig | null };
